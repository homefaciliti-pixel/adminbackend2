...
async function run() {
  ...
  try {
    ...
    const outPath = path.join(__dirname, 'jaipur_partners_list.pdf');
    ...
    doc.fillColor('white')
       .fontSize(20)
       .font('Helvetica-Bold')
       .text('HomeFaciliti — Jaipur Partners', 40, 20, { align: 'center' });
    ...
    // ========================
    let y = 100;
    doc.rect(40, y, pageW, 36).fill('#e8eaf6');
    doc.fillColor('#1a237e').fontSize(11).font('Helvetica-Bold')
    ...
    function drawHeader(startY) {
      doc.rect(40, startY, pageW, rowH).fill('#3949ab');
      let x = 40;
      heads.forEach((h, i) => {
    ...
    allRows.forEach((p, idx) => {
      ...
      const bg = idx % 2 === 0 ? '#ffffff' : '#f3f4ff';
    ...
    y += 15;
    doc.rect(40, y, pageW, 30).fill('#1a237e');
    doc.fillColor('white').fontSize(10).font('Helvetica-Bold')
       .text(
         `Grand Total: ${allRows.length} Partners in Jaipur  |  All Approved ✓`,
    ...
    console.log(`Total partners: ${allRows.length}`);
