  ...
  try {
    ...
    if (nodeRows.length > 0) {
      const [res] = await pool.query(
        "UPDATE node_partners SET isApproved = 1, isPaid = 1 WHERE mobile LIKE ?",
        [`%${mobile}%`]
      );
    ...
    if (laravelRows.length > 0) {
      const [res] = await pool.query(
        "UPDATE homef4fw_homefaci.users SET is_approval = 1, payment_status = 1 WHERE mobile_number LIKE ? AND role_id = 2",
        [`%${mobile}%`]
      );
