...
async function getAllOrders(req) {
  ...
  const [
    ...
    db.query('SELECT id, serviceRequestNumber, serviceName, serviceAmount, slotTime, serviceDate, city, locality, status, vendorName, vendorMobile, createdAt, paymentMethod, latitude, longitude FROM orders').catch(err => { console.warn('Order fetch orders error:', err.message); return [[]]; }),
    // db.query('SELECT * FROM node_orders_v2').catch(err => { console.warn('Order fetch nodeV2Orders error:', err.message); return [[]]; }),
    db.query('SELECT id, userPhone, serviceName, price, date, status, bookingStatus, partnerName, partnerPhone, timeSlot, address, payment, createdAt FROM node_orders_v2').catch(err => { console.warn('Order fetch nodeV2Orders error:', err.message); return [[]]; }),
