...
async function getAllPartners(req) {
  const dbName = process.env.DB_NAME || 'homef4fw_homefaci';
  ...
  // Fetch with pagination limits to stop memory bloat and database lag
  const [
    [nodeRows],
    [laravelRows]
  ] = await Promise.all([
...
// GET all partners (Optimized)
router.get('/', async (req, res) => {
  try {
    let list = await getAllPartners(req);
    res.json({
      success: true,
      data: list.map(p => mapPartner(p, req)),
      total: list.length
